...
(function(){
  ...
  */
  DGJS.Graph.prototype.addNode = function(name, value, attrs) {
    var new_node = new DGJS.Node(name, value, attrs, this);
  ...
  */
  DGJS.Graph.prototype.findPath = function(start_node, end_node) {
    ...
    for (var i = 0; i < this.nodes.length; i++) {
      var clone_and_score = DGJS.Utility.extend({
        f: 0,
        g: 0,
        h: 0
      }, this.nodes[i]);
    ...
    while (DGJS.Utility.length(open_list) > 0) {
      var lowest_id = 0;
      for (var id in open_list) {
        var $id = parseInt(id);
        if (DGJS.Utility.type(open_list[lowest_id]) == 'undefined') { lowest_id = $id }
      ...
      for (var i = 0; i < neighbors.length; i++) {
        ...
        if (!open_list[neighbor.id]) {
          ...
          is_best_g = true;
          neighbor.h = 1;
          if (DGJS.Utility.type(neighbor) =='undefined') {debugger}
